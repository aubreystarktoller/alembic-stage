...
test:
	python setup.py test --pytest-args="--cov=alembic_stage --cov-config coverage.${python_version_major}.rc"

install: clean
	python setup.py install

lint:
	pylint --rcfile=pylint.rc alembic_stage

lint-tests:
	pylint --rcfile=tests/pylintrc --disable I tests/*.py 

lint-bare:
	pylint --rcfile=pylint.rc --disable C,R --persistent=n --reports=n alembic_stage
